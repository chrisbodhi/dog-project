...
- In order to see the emoji in the output of the final cell of the Jupyter Notebook, install the [Symbola](fonts/Symbola) font as a system font (for instance, using Font Book on macOS), trash Matplotlib's font cache file (probably located at `~/.matplotlib/fontList.py3k.cache`, again for macOS), and start the Jupyter Notebook. The font should be available and if it is, it'll be used to render the final graph. If not, the default font will be used and all of those silly emoji will show up as empty rectangles of sadness.
